...
TransOrder.update = function (id, transOrder, result) {
    dbConn.query("UPDATE trans_order SET order_no=?,order_item=?,volume=?,point_token=?,username=?,admin_approve=?,approve_date=?,address=? WHERE id = ?", [transOrder.order_no,transOrder.order_item,transOrder.volume,transOrder.point_token,transOrder.username,transOrder.admin_approve,transOrder.approve_date,transOrder.address, id], function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
        } else {
